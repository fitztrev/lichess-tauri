...
<template>
  <div class="w-3/4 mx-auto my-20">
    <router-link to="/engines/new" class="btn btn-circle btn-accent float-right">
    ...
    <div class="card w-3/4 bg-base-200 shadow-xl mb-8" v-for="engine in engines.engines">
      <div class="card-body">
        <h2 class="card-title">{{ engine.name }}</h2>
        <div>
          <span class="badge badge-primary">{{ engine.maxThreads }} threads</span>
          <span class="badge badge-primary">{{ engine.maxHash }} MB</span>
          <span class="badge badge-primary">{{ engine.defaultDepth }} max depth</span>
          <span v-for="variant in engine.variants">
            <span class="badge badge-accent badge-outline">{{ variant }}</span>
          </span>
        </div>
        <a href="#" class="text-sm font-mono" @click.prevent="openContainingFolder(engine.binaryLocation)">
